    ...
    def __init__(
        ...
        try:
            ...
            if self.qdrant.collection_exists(self.collection_name):
                ...
                log.warning(
                    f"Коллекция Qdrant '{self.collection_name}' не найдена — "
                    f"работаем без референсов (ingest_data.py не запускался)."
                )
        except Exception as e:
            log.warning(f"Qdrant недоступен, работаем без референсов: {e}")
